...
export const formatDate = (arr: (TProfileDB | TProfileCheckDB)[]) => {
  arr.forEach((el, idx) => {
    for (let k in el) {
      for (let i = 0; i < DATE_FIELDS.length; i++) {
        if (
          ...
          if (el[k as keyof (TProfileDB | TProfileCheckDB)]) {
            // @ts-ignore
            let stringToDateField: TProfileDB = new Date(el[k]) as unknown as TProfileDB;
